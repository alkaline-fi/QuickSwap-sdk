...
describe('Pair', () => {
  const USDC = new Token(ChainId.LINEA_TESTNET, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', 18, 'USDC', 'USD Coin')
  const DAI = new Token(ChainId.LINEA_TESTNET, '0x6B175474E89094C44Da98b954EedeAC495271d0F', 18, 'DAI', 'DAI Stablecoin')

  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(() => new Pair(new TokenAmount(USDC, '100'), new TokenAmount(WETH[ChainId.LINEA_TESTNET], '100'))).toThrow(
        'CHAIN_IDS'
      )
    })
  })
  ...
  describe('#priceOf', () => {
    ...
    it('throws if invalid token', () => {
      expect(() => pair.priceOf(WETH[ChainId.LINEA_TESTNET])).toThrow('TOKEN')
    })
  })
  ...
  describe('#reserveOf', () => {
    ...
    it('throws if not in the pair', () => {
      expect(() =>
        new Pair(new TokenAmount(DAI, '101'), new TokenAmount(USDC, '100')).reserveOf(WETH[ChainId.LINEA_TESTNET])
      ).toThrow('TOKEN')
    })
  })
  ...
  describe('#chainId', () => {
    it('returns the token0 chainId', () => {
      expect(new Pair(new TokenAmount(USDC, '100'), new TokenAmount(DAI, '100')).chainId).toEqual(ChainId.LINEA_TESTNET)
      expect(new Pair(new TokenAmount(DAI, '100'), new TokenAmount(USDC, '100')).chainId).toEqual(ChainId.LINEA_TESTNET)
    })
  })
  describe('#involvesToken', () => {
    ...
    expect(
      new Pair(new TokenAmount(USDC, '100'), new TokenAmount(DAI, '100')).involvesToken(WETH[ChainId.LINEA_TESTNET])
    ).toEqual(false)
  })
})
